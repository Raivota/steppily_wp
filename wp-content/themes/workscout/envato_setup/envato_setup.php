...
?>
			...
			<body class="envato-setup wp-core-ui">
				<h1 id="wc-logo">
					...
					<a href="https://themeforest.net/user/purethemes" target="_blank"><?php
		...
		public function _content_install_settings()
		{
			...
			$postajob = get_page_by_title('Post a Job');
			if ($postajob) {
				update_option('job_manager_submit_job_form_page_id', $postajob->ID);
			}

			$jobdashboard = get_page_by_title('Job Dashboard');
			if ($jobdashboard) {
				update_option('job_manager_job_dashboard_page_id', $jobdashboard->ID);
			}
			...
			$postaresume = get_page_by_title('Submit Resume');
			if ($postaresume) {
				update_option('resume_manager_submit_resume_form_page_id', $postaresume->ID);
			}

			$candidatedashboard = get_page_by_title('Candidate Dashboard');
			if ($candidatedashboard) {
				update_option('resume_manager_candidate_dashboard_page_id', $candidatedashboard->ID);
			}

			$resumes = get_page_by_title('Resumes');
			if ($resumes) {
				update_option('resume_manager_resumes_page_id', $resumes->ID);
			}

			$workscout_dashboard_page = get_page_by_title('Dashboard page');
			if ($workscout_dashboard_page) {
				update_option('workscout_dashboard_page', $workscout_dashboard_page->ID);
			}

			$messages_page = get_page_by_title('Messages');
			if ($messages_page) {
				update_option('workscout_messages_page', $messages_page->ID);
			}

			$profile_page = get_page_by_title('My Profile');
			if ($profile_page) {
				update_option('workscout_profile_page', $profile_page->ID);
			}
			$lost_password_page = get_page_by_title('Lost Password');
			if ($lost_password_page) {
				update_option('workscout_lost_password_page', $lost_password_page->ID);
			}

			$reset_password_page = get_page_by_title('Reset Password');
			if ($reset_password_page) {
				update_option('workscout_reset_password_page', $reset_password_page->ID);
			}

			$categories_page = get_page_by_title('Browse Categories');
			if ($categories_page) {
				update_option('workscout_categories_page', $categories_page->ID);
			}

			$alerts_page = get_page_by_title('Job Alerts');
			if ($alerts_page) {
				update_option('job_manager_alerts_page_id', $alerts_page->ID);
			}
			$bookmarks_page = get_page_by_title('My Bookmarks');
			if ($bookmarks_page) {
				update_option('pp_bookmarks_page', $bookmarks_page->ID);
			}
			$past_app_page = get_page_by_title('Past Applications');
			if ($past_app_page) {
				update_option('workscout_past_applications', $past_app_page->ID);
			}

			//job_manager_companies_page_id
			$companies_page = get_page_by_title('Companies');
			if ($companies_page) {
				update_option('job_manager_companies_page_id', $companies_page->ID);
			}

			//job_manager_companies_page_id
			$submit_company_page = get_page_by_title('Submit Company');
			if ($submit_company_page) {
				update_option('job_manager_submit_company_form_page_id', $submit_company_page->ID);
			}
			$manage_company_page = get_page_by_title('Company Dashboard');
			if ($manage_company_page) {
				update_option('job_manager_company_dashboard_page_id', $manage_company_page->ID);
			}
			...
			// 	update_option( 'workscout_resume_categories_page', $resume_categories_page->ID );
			// }

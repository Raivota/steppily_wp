...
if($map) { 
	$all_map = Kirki::get_option( 'workscout', 'pp_enable_all_tasks_map', 0 ); 
	if($all_map){ 
		echo do_shortcode('[workscout-map type="tasks" class="tasks_page"]'); 
	} else { ?>
		<div id="search_map" class="tasks_map"></div>
	<?php 
	}
} ?>
...
<div class="container  wpjm-container <?php echo esc_attr($layout); ?>">
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('eleven columns'); ?>>
		<div class="padding-right">
			<?php 
			$search_in_sb =  Kirki::get_option( 'workscout','pp_tasks_search_in_sb');
			if(!$search_in_sb) {
				...
				?>
				<?php if ( apply_filters( 'workscout_tasks_show_search_keywords', true ) ): ?>
				<form class="list-search"  method="GET" action="<?php echo get_permalink(); ?>">
					<div class="search_keywords">
						<button><i class="fa fa-search"></i></button>
						<input type="text" name="search_keywords" id="search_keywords" placeholder="<?php esc_attr_e( 'task title, keywords or company name', 'workscout' ); ?>" value="<?php echo esc_attr( $keywords ); ?>" />

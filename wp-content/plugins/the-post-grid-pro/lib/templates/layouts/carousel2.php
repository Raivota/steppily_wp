...
if(in_array('title', $items)){
    $titleHtml .= "<h3 class='entry-title'><a data-id='{$pID}' class='{$anchorClass}' {$link_target} href='{$pLink}'>{$title}</a></h3>";
}
...
$html .= "<div class='{$grid} {$class}' data-id='{$pID}'>";
    $html .= '<div class="rt-holder">';
        ...
        $html .= '</div> ';
		if($imgSrc) {
			$html .= "<a href='{$pLink}' data-id='{$pID}' class='{$anchorClass}'{$link_target}><img class='rt-img-responsive' src='{$imgSrc}' alt='{$title}'></a>";
		}
    $html .= '</div>';
$html .='</div>';

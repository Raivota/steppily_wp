...
if(in_array('author', $items)){
    $metaHtmlbottom .= "<span class='author'><i class='fa fa-user'></i>{$author}</span>";
 }
if(in_array('categories', $items) && $categories){
    $metaHtmltop .= "<span class='categories-links'><i class='fa fa-folder-open-o'></i>{$categories}</span>";
}
if(in_array('tags', $items) && $tags){
    $metaHtmlbottom .= "<span class='post-tags-links'><i class='fa fa-tags'></i>{$tags}</span>";
}
$num_comments = get_comments_number(); // get_comments_number returns only a numeric value
if(in_array('comment_count', $items) && $comment){
    $metaHtmlbottom .= '<span class="comment-count"><a href="' . get_comments_link() .'"><i class="fa fa-comments-o"></i> '. $num_comments.'</a></span>';
}
$postMetaBottom = null;
...
$html .= "<div class='{$grid} {$class}' data-id='{$pID}'>";
	$html .= sprintf('<div class="rt-holder%s">',$tpg_title_position ? " rt-with-title" : null);
        ...
        $html .="<div class='rt-detail'>";
            ...
            $html .= "<div class='post-meta-user'>{$metaHtmltop}</div>";

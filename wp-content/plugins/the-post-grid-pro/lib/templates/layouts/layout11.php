<?php
$html = $htmlDetail = $write_comments = $metaHtml = $iTitle = null;
...
$html .= "<div class='{$grid} {$class}' data-id='{$pID}'>";
    $html .= '<div class="rt-holder">';
		$html .= $iTitle;
        if(!empty($metaHtml)){
                $html .= "<div class='post-meta-user'><span class='meta-data'>{$metaHtml}</span></div>";
            }
		if($imgSrc) {
			$html .= "<div class='rt-img-holder'><a data-id='{$pID}' class='{$anchorClass}' href='{$pLink}'{$link_target}><img class='rt-img-responsive' src='{$imgSrc}' alt='{$title}'></a></div>";
		}

			...
			if ( rtTPG()->verifyNonce() ) {
				...
				if ( $scID && ! is_null( get_post( $scID ) ) ) {
					...
					if ( $teamQuery->have_posts() ) {
						...
						$not_found_text = isset( $scMeta['tgp_not_found_text'][0] ) && ! empty( $scMeta['tgp_not_found_text'][0] ) ? esc_attr( $scMeta['tgp_not_found_text'][0] ) : __( 'No post found', 'the-post-grid-pro' );
					...
					$msg = apply_filters( 'tpg_shortcode_not_found_error_text', __( 'Shortcode Id not defined', 'the-post-grid-pro' ) );
				}
			} else {
				$msg = apply_filters( 'tpg_session_error_text', __( 'Session error', 'the-post-grid-pro' ) );
			}

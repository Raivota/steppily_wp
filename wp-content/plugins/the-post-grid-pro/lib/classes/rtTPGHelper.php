	...
	class rtTPGHelper {
		...
		function rtShare( $pid ) {
			...
			if ( in_array( 'reddit', $ssList ) ) {
				...
				$html  .= "<a class='pinterest' title='" . __( "Share on reddit",
						"the-post-grid-pro" ) . "' target='_blank' href='http://reddit.com/submit?url={$permalink}&amp;title={$title}'><i class='fa fa-reddit-alien' aria-hidden='true'></i></a>";
			}
			...
			if ( in_array( 'email', $ssList ) ) {
				...
				$html    .= sprintf( '<a class="email" title="%s" href="mailto:?subject=%s&body=%s"><i class="fa fa-envelope"></i></a>',
					__( "Share on Email", "testimonial-slider-showcase-pro" ),
					$title,
					$content
				);
			}
		...
		public function layoutStyle( $layoutID, $scMeta, $layout, $scId = null ) {
			...
			if ( $scId ) {
				...
				$button_text_color              = ( isset( $scMeta['button_text_bg_color'] ) ? $scMeta['button_text_bg_color'] : null );
			...
			if ( $excerpt_color || $excerpt_size || $excerpt_weight || $excerpt_alignment ) {
				$css .= "#{$layoutID} .{$layout} .rt-holder .tpg-excerpt,#{$layoutID} .rt-holder .rt-woo-info p{";
				if ( $excerpt_color ) {

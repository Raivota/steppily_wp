		...
		function the_post_grid_short_code( $atts, $content = null ) {
			...
			if ( $scID && ! is_null( get_post( $scID ) ) ) {
				...
				if ( ! empty( $filters ) && ( $isGrid || $isOffset || $isWooCom || $isEdd ) ) {
					...
					$allText                   = apply_filters( 'tpg_filter_all_text', __( "All", "the-post-grid-pro" ), $scMeta );
					...
					if ( in_array( '_search', $filters ) ) {
						...
						$html .= sprintf( '<input type="text" class="rt-search-input" placeholder="%s">', esc_html__( "Search...", 'the-post-grid-pro' ) );
